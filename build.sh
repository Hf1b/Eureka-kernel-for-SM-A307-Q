...
AROMA_ZIP()
{
	...
	if [ -e "arch/$ARCH/boot/Image" ]; then
		...
		if [ "${TYPE}" == "oneui" ]; then                     
			...
			if [ "${SELINUX_B}" == "enforcing" ]; then
				AROMA_DIR="gsi_enf"
				AROMA_ZIP="gsi_enf.zip"
			elif [ "${SELINUX_B}" == "permissive" ]; then
				AROMA_DIR="gsi_perm"
...
SELINUX()
{
	...
	case $choice in
		...
		*)
			echo
			echo "Invalid version entered. Exiting..."
			sleep 2
			exit 1
			;;
	esac
...
OS_MENU()
{
	...
	if [ ! -z "$FORCEANDROID" ] && [[ "$FORCEANDROID" =~ ^[0-9]+$ ]] && (( "$FORCEANDROID" > -1 )) && (( "$FORCEANDROID" < "${#menuos[@]}" )); then
		menuos=${menuos[$FORCEANDROID]}
		ANDSEL
	else
		echo " "
...
Please select your Android Version: '
		...
		select menuos in "${menuos[@]}"; do ANDSEL; break; done
	fi
...
devices=("$SM_A105X" "$SM_A205X" "$SM_A305X" "$SM_A307X" "$SM_A405X" "$SM_A505X" "Exit")
...
if [ ! -z "$FORCEDEVICE" ] && [[ "$FORCEDEVICE" =~ ^[0-9]+$ ]] && (( "$FORCEDEVICE" > -1 )) && (( "$FORCEDEVICE" < "${#devices[@]}" )); then
...
Please select your device: '
	...
	select device in "${devices[@]}"; do DEVSEL; break; done
fi
